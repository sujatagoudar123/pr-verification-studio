...
function analyzeSentiment(text) {
  ...
  for (const [w] of SENTIMENT.positive.strong.map(w=>[w])) { if (t.includes(w)) { score+=3; signals.push({word:w,type:"positive",weight:"strong"}); }}
  for (const [w] of SENTIMENT.positive.moderate.map(w=>[w])) { if (t.includes(w)) { score+=2; signals.push({word:w,type:"positive",weight:"moderate"}); }}
  for (const [w] of SENTIMENT.positive.weak.map(w=>[w])) { if (t.includes(w)) { score+=1; signals.push({word:w,type:"positive",weight:"weak"}); }}
  for (const [w] of SENTIMENT.negative.strong.map(w=>[w])) { if (t.includes(w)) { score-=3; signals.push({word:w,type:"negative",weight:"strong"}); }}
  for (const [w] of SENTIMENT.negative.moderate.map(w=>[w])) { if (t.includes(w)) { score-=2; signals.push({word:w,type:"negative",weight:"moderate"}); }}
  for (const [w] of SENTIMENT.negative.weak.map(w=>[w])) { if (t.includes(w)) { score-=1; signals.push({word:w,type:"negative",weight:"weak"}); }}
...
function Summary({results}){
  ...
  return <div style={{background:C.card,borderRadius:16,padding:24,marginBottom:20,border:`1px solid ${C.border}`,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
